...
public class Amazons extends GamePlayer{
    ...
    public GameConstraints localContraints = new GameConstraints();
    /**
     * Constructor
     * @param name
     * @param passwd
	...
	this.gameClient.joinRoom(rooms.get(12));	 		
    }
	...
	//Add update to local game board (for opponents move)
	//Integer[] opponentMove = {qnew.get(0),qnew.get(1),arrow.get(0),arrow.get(1),qcurr.get(0),qcurr.get(1)};
	//Note: we do not need a side specification (i.e. for black or white queen moves) b/c only one queen can occupy a single space at any given time
	//localContraints.updateBoard(opponentMove);
	...
	//Task: Replace the above with a timed task to wait for 10 seconds before sending the move
	java.util.Timer timer = new java.util.Timer();
	//int[] qf1 = {3, 1};
    //int[] qn1 = {4, 1};
    //int[] ar1 = {6, 1};
    
    MyTimer timertask = new MyTimer(gameClient,qf,qn,ar);
    System.out.println("Start Task");
    
    //TODO: Add an if statement to send the next move if the opponents move is registered early (rather than always waiting 10 seconds to update)
     ...
     * @param qfc queen original col
	    ...
	    public boolean markPosition(int qrow, int qcol, int arow, int acol, 
		    ...
		    //Add update to local game board (for our move)
		    Integer[] ourMove = {qrow, qcol, arow, acol, qfr, qfc};
			//Note: we do not need a side specification (i.e. for black or white queen moves) b/c only one queen can occupy a single space at any given time
			localContraints.updateBoard(ourMove);
